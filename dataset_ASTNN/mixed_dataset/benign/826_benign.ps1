...
New-CronJob -Command 'python -c ~/scripts/backup_users <-||- ; 
 -||-> New-CronJob -Command 'powershell -c "cd ~/src/PowerShell; ipmo ./build.psm1; Start-PSBuild"; 
...
Get-CronJob | Where-Object { $_.Command -match ' <-||- 
