...
param(
 ...
 -||-> function Clear-Form {
    ...
    if ( -||-> $TextBoxComputerName.Background -like "
        $TextBoxComputerName.Background = " <-||- 
    } <-||- 
    if ( -||-> $TextBoxMACAddress.Background -like "
        $TextBoxMACAddress.Background = " <-||- 
    }
    if ( -||-> $TextBoxPrimaryUser.Background -like "
        $TextBoxPrimaryUser.Background = " <-||- 
    }
}
...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml" Title="Prestage Computer 1.0.0" Height="453" Width="533" ResizeMode="NoResize">
    <Grid>
        ...
        <StatusBar x:Name="StatusBar" HorizontalAlignment="Left" Height="22" Margin="0,392,0,0" VerticalAlignment="Top" Width="529" BorderBrush="
...
"@ <-||- 
 ...
 -||-> $TextBoxPrimaryUserEvent = $TextBoxPrimaryUser.Add_GotFocus({
    if ( -||-> $TextBoxPrimaryUser.Background.Color -like "
        $TextBoxPrimaryUser.Background = " <-||- 
