             ...
             -||-> if ( -||-> $OpenFileDialogCSV.FileName.Length -ge 1 <-||- ) {
                 ...
                 -||-> foreach ($OSObject in  -||-> $OSObjects <-||- ) {
                     -||-> if ( -||-> Validate-OSPath -OSName $OSObject.Name -OSPath $OSObject.SourcePath <-||- ) {
                         -||-> $DGVOS.Rows.Add($OSObject.Name, $OSObject.ID, $OSObject.SourcePath, $OSObject.RefName, $OSObject.FullName, $OSObject.OrgName, $OSObject.HomePage, $OSObject.AdminPassword) <-||- 
                    } <-||- 
                } <-||- 
...
"@ <-||-  
 ...
 -||-> function Start-Installation {
     ...
     -||-> Write-OutputBox -OutputBoxMessage "Starting to create and configure a WSUS automatic approval rule" -Type INFO <-||- 
     -||-> try {
        
         -||-> $WSUSConnection = [Microsoft.UpdateServices.Administration.AdminProxy]::getUpdateServer($MDTServer,$false,"8530") <-||- 
         -||-> $WSUSRuleName = "Automatic Approval" <-||- 
         -||-> $WSUSRule = $WSUSConnection.CreateInstallApprovalRule($WSUSRuleName) <-||- 
     ...
     -||-> try {
         -||-> $WSUSConnection = [Microsoft.UpdateServices.Administration.AdminProxy]::getUpdateServer($MDTServer,$false,"8530") <-||- 
         -||-> $WSUSAutomaticRule =  -||-> $WSUSConnection.GetInstallApprovalRules() | Where-Object {  -||-> $_.Name -like "Automatic Approval" <-||-  } <-||-  <-||- 
     ...
     -||-> Set-StatusBarText -State Running -Message "MDT Factory installation completed" <-||- 
 ...
 -||-> $Form.Text = "MDT Factory Tool 1.0.0" <-||- 
