...
Add-Content -Value " <-||- 
 -||-> function prompt {
    ...
    $host.UI.RawUI.WindowTitle = "PowerShell v$MajorVersion.$MinorVersion $ShellBits | $env:USERNAME -||-> @ <-||- @$ -||-> e <-||- env:USERDNSDOMAIN | $env:COMPUTERNAME | $env:LOGONSERVER -||-> " <-||- "

    
    Write-Host(Get-Date -UFormat "% -||-> Y <-||- Y/%m/%d %H:%M:%S ( -||-> $howlongwasthat <-||- ) | ") -NoNewline -ForegroundColor DarkGreen
