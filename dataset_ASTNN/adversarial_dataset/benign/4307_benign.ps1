...
If( -||-> ( -||-> $JSON_Convert.'@odata.type' -eq "
    ...
    Write-Host " -||-> A <-||-  <-||- App Configuration JSON is for Managed Devices" -ForegroundColor Yellow
    ...
    ElseIf -||-> ( -||-> $JSON_Convert.'@odata.type' -eq "
        ...
        If($targetedMobileApp){
            ...
            Write-Host
            Write-Host " -||-> T <-||-  <-||- Targeted app $( -||-> $JSON_Convert.packageId <-||- ) has already been added from Managed Google Play" -ForegroundColor Yellow
            Write-Host "The App Configuration Policy will be created" -ForegroundColor Yellow
            Write-Host
