     ...
     -||-> Context 'from module' {
        ...
        finally
        {
         ...
         -||-> It 'generates markdown that includes CommonParameters in advanced functions' {
             -||-> ( -||-> $files | Where-Object -FilterScript {  -||-> $_.Name -eq 'Get-AdvancedFn.md' <-||-  } <-||- ).FullName | Should -FileContentMatch '
        }

        It 'generates markdown that excludes CommonParameters from simple functions' {
            ($files | Where-Object -FilterScript { $_.Name -eq 'Get-SimpleFn.md' }).FullName | Should -FileContentMatch -Not ' <-||- 
        } <-||- 

         -||-> It 'generates markdown for workflows with CommonParameters' -Skip:$IsUnix {
             -||-> ( -||-> $files | Where-Object -FilterScript {  -||-> $_.Name -eq 'FromCommandWorkflow.md' <-||-  } <-||- ).FullName | Should -FileContentMatch '
        }
    }
...
"@
 ...
 -||-> '@
            ...
            ($files | Measure-Object).Count | Should Be 1
            normalizeEnds (Get-Content $files | Where-Object {$_.StartsWith(' <-||- 
        } <-||- 
    } <-||- 
...
'@ <-||- 
 ...
 -||-> Describe 'Update-MarkdownHelp reflection scenario' {
     ...
     -||-> function global:Get-MyCoolStuff
    {
     ...
     -||-> It 'produce a dummy example' {
         -||-> $v1md.FullName | Should FileContentMatch '
    }
 ...
 -||-> '@)
    ...
    It ' -||-> h <-||- has a placeholder for example' {
 ...
 -||-> '@
        normalizeEnds($v2md | Get-Content | Where-Object {$_.StartsWith(' -||-> T <-||- Type: ')} | Out-String) | Should Be $expectedParameters
 ...
 -||-> '@
        normalizeEnds($v3markdown | Where-Object {$_.StartsWith(' -||-> T <-||- Type: ')} | Out-String) | Should Be $expectedParameters
...
If  -||-> the path includes escape characters, enclose each escape character in single quotation marks, to instruct Windows PowerShell not to interpret any characters as escape sequences. <-||- 
 ...
 -||-> '@
    ...
    It ' -||-> V <-||- Verifies that a bad metadata value for the help file is fixed on update' {
