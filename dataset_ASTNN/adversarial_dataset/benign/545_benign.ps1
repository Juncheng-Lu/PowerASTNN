...
write-host "-- -||-> - <-||- -----------------------------------------------------------------"
Write-Host
Write-Host " -||-> A <-||- App Protection Policies: $OS" -ForegroundColor Cyan
Write-Host
write-host "-------------------------------------------------------------------"
Write-Host
...
$AssignmentCount = 0
    ...
    foreach($ManagedAppPolicy in $ManagedAppPolicies){
        ...
        elseif -||-> ( -||-> $ManagedAppPolicy.'@odata.type' -eq "
            ...
            $iOSManagedAppProtection = Get-ManagedAppProtection -id $ManagedAppPolicy.id -OS " -||-> i <-||-  <-||- iOS"
        ...
        if($ManagedAppConfiguration.assignments){
            ...
            foreach($group in $ManagedAppConfiguration.assignments){
                ...
                foreach($MAMApp in $MAMApps){
                    ...
                    if($MAMApp.mobileAppIdentifier.'@odata.type' -eq " <-||- 
                    ...
                    elseif -||-> ( -||-> $MAMApp.mobileAppIdentifier.'@odata.type' -eq "
