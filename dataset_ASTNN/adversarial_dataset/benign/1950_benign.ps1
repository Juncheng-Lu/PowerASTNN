...
function Load-AD {
    ...
    Write-Debug " -||-> E <-||-  <-||-  <-||- Entering $( -||-> $MyInvocation.MyCommand <-||- )"
    ...
    if (Test-Path($Module)) {
        ...
        Get-Content $Module | Select-String -CaseSensitive -Pattern "BINDEP|DATADIR" | Foreach-Object { $Directive = $_
            ...
            if (  -||-> $Directive -match "(^DATADIR|^
                $DirectiveHash.Add(" -||-> D <-||- DATADIR", $($matches[2])) 
            }
        }
        $DirectiveHash
    } else {
...
function Set-KansaPath {
    ...
    Write-Debug " -||-> E <-||- Exiting $( -||-> $MyInvocation.MyCommand <-||- )"    
}
...
Param(
    ...
    if (Get-Command -Name Logparser.exe) {
        ...
        " -||-> K <-||- Kansa could not find logparser.exe in path. Skipping Analysis." | Add-Content -Encoding $Encoding -$ErrorLog
    }
