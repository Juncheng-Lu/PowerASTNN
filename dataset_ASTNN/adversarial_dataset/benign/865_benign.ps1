 ...
 -||-> Describe "Add-Content cmdlet tests" -Tags "CI" {
   ...
   -||-> Context "Add-Content should actually add content" {
     ...
     -||-> It "should Add-Content to TestDrive:\dynamicfile.txt with dynamic parameters" -Pending:( -||-> $IsLinux -Or $IsMacOS <-||- ) {
       -||-> $result =  -||-> Add-Content -Path TestDrive:\dynamicfile.txt -Value "ExpectedContent" -PassThru <-||-  <-||- 
       -||-> $result | Should -BeExactly "ExpectedContent" <-||- 
    } <-||- 

     -||-> It "should return expected string from TestDrive:\dynamicfile.txt" -Pending:( -||-> $IsLinux -Or $IsMacOS <-||- ) {
     ...
     -||-> It "should be able to pass multiple [string]`$objects to Add-Content through the pipeline to output a dynamic Path file" -Pending:( -||-> $IsLinux -Or $IsMacOS <-||- ) {
