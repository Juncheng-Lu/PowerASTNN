...
function ExecuteBlock {
    ...
    $splat = @{
        ...
        'R e p o r t S c o p e' = { param ($CommandName, $ModuleName, $ScriptBlock)
            Write-ScriptBlockInvocationHint -Hint " -||-> M <-||- Mock - of command $CommandName$( -||-> if ( -||-> $ModuleName <-||- ) {  -||-> "from module $ModuleName" <-||- } <-||- )" -ScriptBlock $ScriptBlock }
    }
...
function Test-ParameterFilter {
    ...
    $scriptBlockString = " <-||- 
         ...
         -||-> $ScriptBlock <-||- 
     -||-> "
    Write-Hint " -||-> U <-||- Unbinding ScriptBlock from '$(Get-ScriptBlockHint $ScriptBlock)'"
...
function Get-DynamicParametersForMockedFunction {
    ...
    if (-not $mock) {
        throw " -||-> I <-||- Internal error detected:  Mock for '$FunctionName' in module '$ModuleName' was called, but does not exist in the mock table."
    }
