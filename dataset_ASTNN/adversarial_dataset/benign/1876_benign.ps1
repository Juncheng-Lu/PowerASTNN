...
'@ <-||- 
    ...
    PROCESS
    {
        ...
        for ( -||-> $i = 0 <-||- ;  -||-> $i -lt $CoffHeader.NumberOfSymbols <-||- ;  -||-> $i++ <-||- )
        {
            ...
            {
                 -||-> $TempPosition = $FileStream.Position <-||-  
                 ...
                 -||-> $FileStream.Seek($TempPosition, 'Begin') | Out-Null <-||-  
            }
        ...
        for ( -||-> $i = 0 <-||- ;  -||-> $i -lt $CoffHeader.NumberOfSymbols <-||- ;  -||-> $i++ <-||- )
        {
             -||-> $SymbolTable[$j] = $RawSymbolTable[$i] <-||-  
             -||-> $j++ <-||- 
        ...
        for ( -||-> $i = 0 <-||- ;  -||-> $i -lt $CoffHeader.NumberOfSections <-||- ;  -||-> $i++ <-||- )
        {
             -||-> if ( -||-> $SectionHeaders[$i].Name -and ( -||-> $SectionHeaders[$i].Name.IndexOf('/') -eq 0 <-||- ) <-||- )
            {
        ...
        {
             -||-> if ( -||-> $SymbolTable[$i].Name -and ( -||-> $SymbolTable[$i].Name.IndexOf('/') -eq 0 <-||- ) <-||- )
            {
