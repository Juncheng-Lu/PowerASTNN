...
$MsiUpgradeCode
     ...
     -||-> if( -||-> $MSI <-||- ){
	     ...
	     -||-> $body = @{ "@odata.type" =  -||-> "
        $body.applicableArchitectures =  <-||- " -||-> x <-||- x64,x86";
...
    [ValidateNotNullOrEmpty()]
	...
	try	{
        ...
        if( -||-> $DetectionRules.'@odata.type' -contains "
            ...
            Write-Host
            Write-Warning " -||-> A <-||- A Detection Rule can either be 'Manually configure detection rules' or 'Use a custom detection script'"
