 ...
 -||-> InModuleScope Pester {
     -||-> Describe "Write xml test results" {
         ...
         -||-> Context 'nunit' {
             ...
             -||-> it "should write two test-suite elements for two describes" {
                 ...
                 -||-> $TestResults =  -||-> New-PesterState -Path TestDrive:\ <-||-  <-||- 
                 -||-> $TestResults.EnterTestGroup('Describe 
                $TestResults.EnterTest()
                Start-Sleep -Milliseconds 200
                $TestResults.LeaveTest()
                $TestResults.AddTestResult("Successful testcase", ' -||-> P <-||- Passed', $null)
                $TestResults.LeaveTestGroup('Describe <-||-  
                 -||-> $Describe1 = $testResults.TestGroupStack.peek().Actions.ToArray()[-1] <-||- 
                 -||-> $testResults.EnterTestGroup('Describe 
                $TestResults.EnterTest()
                Start-Sleep -Milliseconds 200
                $TestResults.LeaveTest()
                $TestResults.AddTestResult("Failed testcase", ' -||-> F <-||- Failed', $null)
                $TestResults.LeaveTestGroup('Describe <-||-  
                 ...
                 -||-> $xmlTestSuite1.name | Should -Be "Describe 
                $xmlTestSuite1.description | Should -Be "Describe <-||-  
                 ...
                 -||-> $xmlTestSuite2.name | Should -Be "Describe 
                $xmlTestSuite2.description | Should -Be "Describe <-||-  
             ...
             -||-> it "Should validate test results against the nunit 2.5 schema" {
                 ...
                 -||-> $TestResults =  -||-> New-PesterState -Path TestDrive:\ <-||-  <-||- 
                 -||-> $testResults.EnterTestGroup('Describe 
                $TestResults.AddTestResult("Successful testcase", ' -||-> P <-||- Passed', (New-TimeSpan -mi 1))
                $testResults.LeaveTestGroup('Describe <-||-  
                 -||-> $testResults.EnterTestGroup('Describe 
                $TestResults.AddTestResult("Failed testcase", ' -||-> F <-||- Failed', (New-TimeSpan -Seconds 2))
            ...
            it "handles special characters in block descriptions well -!@
                
                $TestResults = New-PesterState -Path TestDrive:\
                $testResults.EnterTestGroup('Describe -!@ <-||- 
                 -||-> $TestResults.AddTestResult("Successful testcase -!@
                $TestResults.LeaveTestGroup('Describe -!@

                
                [String]$testFile = " -||-> $ <-||- $TestDrive -||-> { <-||- { -||-> 0 <-||- } -||-> R <-||- Results{ -||-> 0 <-||- }Tests.xml" -f [System.IO.Path]::DirectorySeparatorChar
                Export-XmlReport $testResults $testFile 'NUnitXml'
                $xml = [xml] (Get-Content $testFile)
    ...
    Describe "Get-TestTime" {
        ...
        It "output is culture agnostic" {
            ...
            $TestResult = New-Object -TypeName psObject -Property @{ Time = [timespan]35000000 } 
