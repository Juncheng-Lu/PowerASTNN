 ...
 -||-> try {
     ...
     -||-> Describe "CIM Objects are adapted properly" -Tag @( -||-> "CI" <-||- ) {
         -||-> BeforeAll {
             ...
             -||-> $indexOf_className_Win32Process            =  -||-> getIndex $p.PSTypeNames "*
            $indexOf_className_CimProcess              = getIndex $p.PSTypeNames "* <-||-  <-||- 
             -||-> $indexOf_className_CimLogicalElement       =  -||-> getIndex $p.PSTypeNames "*
            $indexOf_className_CimManagedSystemElement = getIndex $p.PSTypeNames "* <-||-  <-||- 
        } <-||- 
         -||-> AfterAll {
