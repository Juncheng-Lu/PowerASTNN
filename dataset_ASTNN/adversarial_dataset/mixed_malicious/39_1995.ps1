     ...
     -||-> Context "File name completion" {
         -||-> BeforeAll {
             ...
             -||-> $testCases = @(
                 ...
                 -||-> @{ inputStr =  -||-> "bbbbbbbbb" <-||- ; name =  -||-> "bbbbbbbbb
                @{ inputStr =  <-||- " -||-> b <-||- bbbbbbbb"; name = "bbbbbbbb{ -||-> "; localExpected = " <-||- &  -||-> '.${separator}bbbbbbbb{' -||-> " <-||- "; oneSubExpected = " <-||- &  -||-> '..${separator}bbbbbbbb{' -||-> " <-||- "; twoSubExpected = " <-||- &  -||-> '..${separator}..${separator}bbbbbbbb{' -||-> " <-||- " }
                @{ inputStr = " -||-> b <-||- bbbbbbb"; name = "bbbbbbb <-||- }"; localExpected = " <-||- &  -||-> '.${separator}bbbbbbb}' -||-> " <-||- "; oneSubExpected = " <-||- &  -||-> '..${separator}bbbbbbb}' -||-> " <-||- "; twoSubExpected = " <-||- &  -||-> '..${separator}..${separator}bbbbbbb}' -||-> " <-||- " }
                @{ inputStr = " -||-> b <-||- bbbbbb"; name = "bbbbbb( -||-> "; localExpected = " <-||- & '.${separator}bbbbbb('"; oneSubExpected = " <-||- &  -||-> '..${separator}bbbbbb(' -||-> " <-||- "; twoSubExpected = " <-||- &  -||-> '..${separator}..${separator}bbbbbb(' -||-> " <-||- " }
                @{ inputStr = " -||-> b <-||- bbbbb"; name = "bbbbb <-||- ) -||-> " <-||- "; localExpected = " <-||- &  -||-> '.${separator}bbbbb)' -||-> " <-||- "; oneSubExpected = " <-||- &  -||-> '..${separator}bbbbb)' -||-> " <-||- "; twoSubExpected = " <-||- &  -||-> '..${separator}..${separator}bbbbb)' -||-> " <-||- " }
                @{ inputStr = " -||-> b <-||- bbbb"; name = "bbbb$"; localExpected = " <-||- &  -||-> '.${separator}bbbb$' -||-> " <-||- "; oneSubExpected = " <-||- &  -||-> '..${separator}bbbb$' -||-> " <-||- "; twoSubExpected = " <-||- &  -||-> '..${separator}..${separator}bbbb$' -||-> " <-||- " }
                @{ inputStr = " -||-> b <-||- bbb"; name = "bbb'"; localExpected = "& '.${separator}bbb'''"; oneSubExpected = "& '..${separator}bbb'''"; twoSubExpected = "& '..${separator}..${separator}bbb'''" }
                @{ inputStr = "bb"; name = "bb,"; localExpected = "& '.${separator}bb,'"; oneSubExpected = "& '..${separator}bb,'"; twoSubExpected = "& '..${separator}..${separator}bb,'" }
                @{ inputStr = "b"; name = "b;"; localExpected = "& '.${separator}b <-||- ; -||-> '"; oneSubExpected = "& '..${separator} -||-> b <-||- b <-||- ; -||-> '"; twoSubExpected = "& '..${separator}..${separator} -||-> b <-||- b <-||- ; -||-> '" }
            )
        ...
        It "Test case insensitive file and folder path completing for <type>" -Skip:(!$IsLinux) -TestCases @(
            @{ type = "File"     ; beforeTab = "Get-Content f"; expected = "foo","Foo" },  
            @{ type = "Directory"; beforeTab = "cd f"         ; expected = "Foo" }  
        ) {
    ...
    Context "Miscellaneous completion tests" {
        BeforeAll {
            $testCases = @(
                ...
                @{ inputStr = ' -||-> $ <-||-  <-||- $MaximumHistoryCount.'; expected = ' -||-> C <-||- CompareTo( -||-> '; setup = $null }
                @{ inputStr = ' <-||- $A=[datetime]::now <-||- ; -||-> $A.'; expected = ' -||-> D <-||- Date'; setup = $null }
                @{ inputStr = '$e=$null <-||- ; -||-> try {  -||-> 1/0 <-||-  } catch { -||-> $e=$_ <-||- } <-||- ; -||-> $e.'; expected = ' -||-> C <-||- CategoryInfo'; setup = $null }
                ...
                @{ inputStr = " <-||- &  -||-> 'get-comm"; expected = "' -||-> G <-||- Get-Command'"; setup = $null }
                ...
                @{ inputStr = ' <-||- &  -||-> "$PSHOME\System.Management.Au" -||-> ' <-||- '; expected = "`"$(Join-Path $PSHOME ' -||-> S <-||- System.Management.Automation.dll')`""; setup = $null }
                
                @{ inputStr = 'get-date | ForEach-Object {  -||-> $PSItem.h -||-> ' <-||- '; expected = ' -||-> H <-||- Hour'; setup = $null }
                @{ inputStr = '$a=gps <-||- ; -||-> $a[0].h -||-> ' <-||- '; expected = ' -||-> H <-||- Handle'; setup = $null }
...
'@
    ...
    Context " -||-> L <-||- Local tab completion with AST" {
        BeforeAll {
            $testCases = @(
