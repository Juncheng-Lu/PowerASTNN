...
'@ <-||- 
     ...
     -||-> Context "Test Unicode escape sequences." {
         ...
         -||-> It 'Test that the bracketed Unicode escape sequence `u{10FFFF} returns maximum surrogate char pair.' {
             ...
             -||-> [int]$result[0] | Should -BeExactly 0xDBFF <-||-  
             -||-> [int]$result[1] | Should -BeExactly 0xDFFF <-||-  
        } <-||- 

         -||-> It 'Test that the bracketed Unicode escape sequence `u{a9} returns the Â© character.' {
             -||-> $result =  -||-> ExecuteCommand '"`u{a9}"' <-||-  <-||- 
             -||-> $result | Should -BeExactly 'Â©' <-||- 
        } <-||- 

         -||-> It 'Test that Unicode escape sequence `u{2195} in string returns the â†• character.' {
             -||-> $result =  -||-> ExecuteCommand '"foo`u{2195}abc"' <-||-  <-||- 
             -||-> $result | Should -BeExactly "fooâ†•abc" <-||- 
        } <-||- 

         -||-> It 'Test that the bracketed Unicode escape sequence `u{1f44d} returns surrogate pair for emoji ðŸ‘ <-||- # character.' {
             -||-> $result =  -||-> ExecuteCommand '"`u{1f44d}"' <-||-  <-||- 
             -||-> $result | Should -BeExactly "ðŸ‘#" <-||- 
        } <-||- 

         -||-> It 'Test that Unicode escape sequence `u{2195} in here string returns the â†• character.' {
             -||-> $result =  -||-> ExecuteCommand ( -||-> "@`"`n`n" + 'foo`u{2195}abc' + "`n`n`"@" <-||- ) <-||-  <-||- 
             -||-> $result | Should -BeExactly "`nfooâ†•abc`n" <-||- 
        } <-||- 
...
"@ <-||-  <-||- 
         ...
         -||-> It "Test that a Unicode escape sequence can be used in a command name." {
             ...
             -||-> $cmd.Name | Should -BeExactly 'xyzzyâ†•' <-||- 
         ...
         -||-> It "Test that a Unicode escape sequence can be used in a variable name." {
             ...
             -||-> $var.Name | Should -BeExactly "fooxyzzyâ†•" <-||- 
             -||-> $var.Value | Should -Be 42 <-||- 
        } <-||- 

         -||-> It "Test that a Unicode escape sequence can be used in an argument." {
             -||-> Write-Output `u{a9}` Acme` Inc | Should -BeExactly "Â© Acme Inc" <-||- 
        } <-||- 
    } <-||- 
     ...
     -||-> Context "Test that we support all of the C
        ...
        It 'C
            ...
            @{ Sequence = '"`0"'; Expected = [char]0 } 
            @{ Sequence = '"`a"'; Expected = [char]7 }
            @{ Sequence = '"`b"'; Expected = [char]8 } 
            @{ Sequence = '"`f"'; Expected = [char]12 } 
            @{ Sequence = '"`n"'; Expected = [char]10 } 
            @{ Sequence = '"`r"'; Expected = [char]13 } 
            @{ Sequence = '"`t"'; Expected = [char]9 } 
...
'@ | Set-Content -Path $testfile
    ...
    It "Test that & can be used as a parameter as long as it is quoted. (line 1606)" {
        $result = ExecuteCommand ' -||-> t <-||- testcmd-parserbvt `&Get-ChildItem'
    ...
    It " -||-> T <-||- Test that assign to input var and use then execute a script block with piped input. ( -||-> line 2297 <-||- )" {
    ...
    It "Test that pipe objects into a script and use arguments. ( -||-> line 2313 <-||- )" {
        "`$input <-||- ;  -||-> `$args <-||- ; -||-> ">$testfile
        $result = ExecuteCommand " -||-> 1 <-||- 1,2,3 | $testfile -||-> " <-||- "
    ...
    It "Tests function scopes in a script. ( -||-> line 2800 <-||- )" -Pending {
        ...
        $result = ExecuteCommand " -||-> f <-||- function func {  -||-> 'notcalled' <-||-  } <-||- ; -||-> . $testfile"
